...
1. **View the script contents using the command `cat 1-build.sh`**
...
1. You might notice that the `Dockerfile` is doing the brunt of the work in this script to build the container image itself. **Take a look at this too, using the command `cat Dockerfile`**.
...
1. **Run the script using the command `./1-build.sh`**
...
1. **View your new image in the registry using the command `podman images`**
...
1. **Run the command `cat app-mod-withroute_cr.yaml` that the two instances of `NN` were properly replaced with your user number**.
...
1. **Run the command cat `2-deploy.sh`**.
...
1. **Run this shell script with the command `./2-deploy.sh`**
...
1. **View all of the created objects using the command `oc get all`**
